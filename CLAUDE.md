...
- **Alerts are computed at run time** in `computeAlerts()` and stored on `S.output.alerts` as `{dups, refDups, cellFlags}`. `S.alerts.cols === null` means "auto" (main columns used in conditions); it becomes an explicit `Set` once the user toggles a chip and resets to `null` when a new main file loads. `refreshAlerts()` recomputes without re-running matching when the threshold or chips change.
